const state = { dashboard: null, category: 'briefing', scope: 'all', period: 'week' };
...
function emptyBox(label, searchUrl){
  return `<div class="empty">${label}<br/>무료 RSS에서 바로 잡힌 기사가 적음${searchUrl?`<br/><a href="${searchUrl}" target="_blank" rel="noopener noreferrer">Google 뉴스에서 직접 보기 ↗</a>`:''}</div>`;
}
function renderPreview(id, data){
  const box = $(id); const items = data?.articles || [];
  box.innerHTML = items.length ? items.slice(0,5).map(article).join('') : emptyBox('표시할 관련 기사가 없음', data?.search_url);
}
...
async function loadArticles(){
  ...
  $('boardDesc').textContent = `${state.period==='week'?'최근 7일':'최근 6개월'} · ${state.scope==='all'?'국내+해외':state.scope==='domestic'?'국내':'해외'} · ${m.subtitle}`;
  ...
  try{
    ...
    $('articleList').innerHTML = items.length ? items.map(article).join('') + `<p class="result-note">${items.length}건 표시 · ${safe(data.method,'')}</p>` : emptyBox('조건에 맞는 기사 없음', data.search_url);
  }catch(e){
    clearTimeout(timer);
